...
__kernel void progressive_refine(__constant Sphere* spheres, __constant Shader* shaders, const int num_spheres,
                    const int width, const int height, const float fov, const float3 pos, 
                    const float3 target, const float3 d_up, const float3 d_width, 
                    const float3 bottom_left, __global float3* output, __global unsigned char* img, int sample) {
    int pix_num = get_global_id(0);
    int x_coord = pix_num % width;
    int y_coord = pix_num / width;
    ...
    img[pix_num * 3    ] = (char) (255*CLAMP(r, 0.0f, 1.0f));
    img[pix_num * 3 + 1] = (char) (255*CLAMP(g, 0.0f, 1.0f));
    img[pix_num * 3 + 2] = (char) (255*CLAMP(b, 0.0f, 1.0f));
